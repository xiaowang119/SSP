...
public class NewAmpereMeter extends MyMeter {
    ...
    public NewAmpereMeter(Context context) {
        this(context, null);
    }

    public NewAmpereMeter(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NewAmpereMeter(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        mStrokeWidth = dp2px(Math.round(myWidth/600)+1);
        mLength1 = dp2px(Math.round(8*myWidth/600)+1) + mStrokeWidth;
        mLength2 = mLength1 + dp2px(Math.round(2*myWidth/600)+1);
        mPSRadius = dp2px(Math.round(25*myWidth/600)+1);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mPaint.setTextSize(sp2px(Math.round(16*myWidth/600)+1));
        ...
        //setMeasuredDimension(width, max + getPaddingTop() + getPaddingBottom() - 300);
        ...
        mPaint.setTextSize(sp2px(Math.round(10*myWidth/600)+1));
        mPaint.getTextBounds("0", 0, "0".length(), mRectText);
        mRectFInnerArc.set(
                getPaddingLeft() + mLength2 + mRectText.height() + Math.round(26*myWidth/600),
                getPaddingTop() + mLength2 + mRectText.height() + Math.round(26*myWidth/600),
                getMeasuredWidth() - getPaddingRight() - mLength2 - mRectText.height() - Math.round(26*myWidth/600),
                getMeasuredWidth() - getPaddingBottom() - mLength2 - mRectText.height() - Math.round(26*myWidth/600)
        );

        mPLRadius = mRadius - (mLength2 + mRectText.height());
        //mPLRadius = mRadius - (mLength2 + mRectText.height() + dp2px(Math.round(5*myWidth/600)+1));
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float x1 = (float) (mPadding + mStrokeWidth + mRadius - (mRadius - mLength1 - Math.round(14*myWidth/600)) * cos);
        float y1 = (float) (mPadding + mStrokeWidth + mRadius - (mRadius - mLength1 - Math.round(14*myWidth/600)) * sin);
        ...
        float x2 = (float) (mPadding + mStrokeWidth + mRadius - (mRadius - mLength1 / 2f - Math.round(10*myWidth/600)) * cos);
        float y2 = (float) (mPadding + mStrokeWidth + mRadius - (mRadius - mLength1 / 2f - Math.round(10*myWidth/600)) * sin);
        ...
        if (!TextUtils.isEmpty(mHeaderText)) {
            mPaint.setTextSize(sp2px(Math.round(14*myWidth/600)+1));
         ...
         * 画指针
         */
        float theta = mStartAngle + mSweepAngle * (mRealTimeValue - mMin) / (mMax - mMin); // 指针与水平线夹角
        int d = dp2px(Math.round(5*myWidth/600)+1); // 指针由两个等腰三角形构成，d为共底边长的一半
        mPath.reset();
        float[] p1 = getCoordinatePoint(d, theta - 90);
        mPath.moveTo(p1[0], p1[1]);
        float[] p2 = getCoordinatePoint(mPLRadius, theta);
        mPath.lineTo(p2[0], p2[1]);
        float[] p3 = getCoordinatePoint(d, theta + 90);
        mPath.lineTo(p3[0], p3[1]);
        float[] p4 = getCoordinatePoint(mPSRadius, theta - 180);
        ...
        canvas.drawCircle(mCenterX, mCenterY, dp2px(Math.round(2*myWidth/600)+1), mPaint);
